...
long long int deterministicFunction(long long int x, long long int M);

long long int md5_int(int x);
...
const long long int w = 4;
const int w_s = 6, q = 16;
...
/////////////////////////////////////// the for loop below gives random LWE instances, and for each instance we will try to find collisions.
    for (int num_exp = 0; num_exp < 1; num_exp++) {
        ...
        for (iteration = 0; iteration < 10000000000; iteration++) { //
    ...
    cout << "Final average: " << (double) average / Number << endl;
...
long long int md5_int(int x) {
    ...
    for (int i = 0; i < 8; i++)
        y |= ((unsigned long long) digest[i] << (8 * i));
